...
const GPIO_Pin THUMBPAD_DETECT = {.letter = GPIOB, .num = GPIO_PIN_2};

const GPIO_Pin THUMBPAD_BUTTON_PINS[] = {
    {.letter = GPIOB, .num = GPIO_PIN_10},
    {.letter = GPIOB, .num = GPIO_PIN_11}
};

#define ROW_PINS_NUM sizeof(ROW_PINS) / sizeof(GPIO_Pin)
#define COL_PINS_NUM sizeof(COL_PINS) / sizeof(GPIO_Pin)
#define THUMBPAD_BUTTON_NUM sizeof(THUMBPAD_BUTTON_PINS) / sizeof(GPIO_Pin)
...
KeyFunction KEYBOARD_DEFS[KEYBOARD_LAYERS][ROW_PINS_NUM][COL_PINS_NUM] = {
    { // layer 0 (left)
        ...
        {{.function = LayerSwitchHold, .value = 1},{.function = ModifierMask, .value=KEY_MOD_LCTRL},{.function = ModifierMask, .value = KEY_MOD_LMETA},{.function = ModifierMask, .value = KEY_MOD_LALT},{.value = KEY_ESC},{.value = KEY_UP},{.value = KEY_DOWN},{.value = KEY_SPACE}},
    },
    { // layer 1 (right)
        ...
        {{.value = KEY_SPACE},{.value = KEY_LEFT},{.value = KEY_RIGHT},{.value = KEY_DELETE},{.function = ModifierMask, .value = KEY_MOD_RALT},{.function = ModifierMask, .value = KEY_MOD_RMETA},{.function = ModifierMask, .value = KEY_MOD_RCTRL},{.function = LayerSwitchHold, .value = 0}},
    }
};

const KeyFunction THUMBPAD_DEFS[KEYBOARD_LAYERS][THUMBPAD_BUTTON_NUM] = {
    { // layer 0 (left)
        {.value = KEY_SPACE},
